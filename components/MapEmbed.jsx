	...
	useEffect(() => {
		if (location.length > 0) {
			mapRef.current.flyTo({
				center: [location[0] - 0.002, location[1]],
				zoom: 16,
				essential: true,
			});
		}
	...
	useEffect(() => {
		const newMarkers = item?.map((e) => {
         ...
         c-34.123,0-61.884-27.761-61.884-61.884s27.761-61.884,61.884-61.884C247.104,74.736,274.865,102.497,274.865,136.62z"
				...
				return new goongjs.Marker(el).setLngLat([i.long, i.lat]).addTo(mapRef.current);
			});
		});
		...
		<>
			<div className="bl_app">
					...
					<div className="bl_app-header">
						{open ? (
							<img
								...
								alt="Đóng menu"
								src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABgAAAAYCAYAAADgdz34AAAACXBIWXMAAAsTAAALEwEAmpwYAAAAaUlEQVR4nO3UwQmAMAxA0YynOJDXzlrd43tREIzStAmI9t/bR1OISO8/AROw0l4GBg1YHC4/yhpwqXIS+tlXAkAyy6UAkEyvtADslz/VCsyhwBmR0qxA+Cff5TaiGiB8VYxOSFaXXe+7bX0uftCCYSYTAAAAAElFTkSuQmCC"
							/>
						) : (
							<img
								onClick={showDrawer}
								style={{ cursor: 'pointer' }}
								alt="Mở menu"
								src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABgAAAAYCAYAAADgdz34AAAACXBIWXMAAAsTAAALEwEAmpwYAAAAaElEQVR4nO3UwQ2AIAxAUcbDqfDInGWQb/SoGJpCjUb+GXjQAyHM/hMQgUJ/BVhqgDAuqQFHxtdfeh64C8jeAC2ESqpFp7I3kLqAxuakWGMGVuUldCNSy28EBOevIg5CZD/LMonZR9sA2EZ+8oFe+CMAAAAASUVORK5CYII="
							/>
						)}
						<div className="bl_app-name">
							...
							<div className="bl_app-title">
								<p className="el_app-title">
									{name} <StarFilled style={{ fontSize: '10px', color: 'grey' }} />
								</p>
				...
				<div className="map">
					...
					<Drawer
						title={
							<div className="bl_view">
								<div className="bl_title">
									<p className="el_span">{userEmail}</p>
									<p className="el_span">{formattedTime}</p>
								</div>
							</div>
						}
						placement="left"
								...
								<>
									<div
										...
										<div className="bl_list-layer">
											<Checkbox
												onChange={(event) => onChange(event, e, index)}
												defaultChecked={e.is_active}></Checkbox>
											<h3 className="el_h3">{e?.name ? e?.name : 'Lớp không có tiêu đề'}</h3>
										</div>
         ...
         c-34.123,0-61.884-27.761-61.884-61.884s27.761-61.884,61.884-61.884C247.104,74.736,274.865,102.497,274.865,136.62z"
															...
															<p className="bl_list-p">Tất cả các mục</p>
														</div>
														{e.points.map((i) => (
															<div
																className="bl_list-name"
																key={i.id}>
																{expandedItems[e.id] ? (
																	<p
																		className="bl_list-detail"
																		onClick={() => handleFlyTo(i)}>
																		{i.name ? i.name : 'Chưa có tiêu đề'}
																	</p>
																) : (
																	''
																)}
			...
			<Drawer
				...
				title={
					<div className="detail_title">
						...
						<div
							className="direction"
							onClick={() => handleDirection(detail)}>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								height="22px"
								fill="white"
